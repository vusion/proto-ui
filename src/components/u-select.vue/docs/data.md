...
如果数据量很大或者需要使用过滤等操作时，推荐对数据进行分页，需要使用`data-source`属性。
...
<u-linear-layout>
    <u-select :data="data"
        placeholder="没有使用分页功能">
    </u-select>
    <u-select :data-source="{ data }" :page-size="50"
        placeholder="使用分页加载功能">
    </u-select>
</u-linear-layout>
</template>
<script>
...
<u-linear-layout>
    <u-select :data="data"
        placeholder="使用`data`属性">
    </u-select>
    <u-select :data-source="{ load }" :page-size="20"
        placeholder="使用`data-source`属性">
    </u-select>
</u-linear-layout>
</template>
<script>
...
``` vue
<template>
<u-select :data-source="{ load }" remote-paging></u-select>
</template>
<script>
// 模拟构造数量较多的 500 条远程数据
let remoteData = [];
...
``` vue
<template>
<u-select :data-source="{ data }"
    filterable clearable placeholder="请输入地名"></u-select>
</template>
...
<u-linear-layout>
    <u-select :data-source="{ load }"
        filterable clearable :page-size="20" placeholder="请输入地名"></u-select>
</u-linear-layout>
</template>
<script>
// 模拟构造远程数据
...
<template>
<u-linear-layout>
    <u-select :data-source="{ load }" filterable remote-filtering remote-paging
        placeholder="使用`data-source`属性"></u-select>
</u-linear-layout>
</template>
<script>
// 模拟构造数量较多的 500 条远程数据
